 ...
 *   10 => 55
 */
function getFibonacciNumber(indexFibonacci) {
  const arr = [];
  arr.push(0);
  if (indexFibonacci >= 1) arr.push(1);
  for (let i = 2; i <= indexFibonacci; i += 1) {
    arr.push(arr[i - 1] + arr[i - 2]);
  }
  return arr[indexFibonacci];
}
...
// if (number > 0) return Math.trunc(number) + 1;
...
function getCountOfOddNumbers(number) {
  ...
  for (let i = 1; i <= moduleNumber; i += 1) {
    if (i % 2 === 1) count += 1;
  }
  return count;
}
